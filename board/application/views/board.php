<div class="container">
    ...
    <table class="table table-striped">
        ...
        <tr>
            <td><?= $entry->seq ?></td>
            <td><a href="/index.php/board/detail/<?= $entry->seq ?>"><?= $entry->title ?></a></td>
                ...
                <nav>
                    <ul class="pagination">
                        ...
                        <?php if ($page['startPage'] - $page['pageSize'] > 0) { ?>
                            <li>
                                <!-- 현재 페이지가 4이고 pageSize가 3일 때 <<(이전페이지)버튼을 누르면 자동으로 1페이지에 있는 게시글이 보이도록 설정! -->
                                <a href="/index.php/board/board/<?= $page['startPage'] - $page['pageSize'] ?>" aria-label="Previous">
                        ...
                        <?php for ($i = $page['startPage']; $i <= $page['endPage']; $i++) { ?>
                            <?php if ($i == $page['nowPage']) { ?>
                                ...
                                <li class="active"><a href="/index.php/board/board/<?= $page['nowPage'] ?>"><?= $page['nowPage'] ?><span class="sr-only">(current)</span></a></li>
                            <?php }
                            else { ?>
                                <!-- 현재 클릭한 페이지를 제외한 나머지는 하얀색 백그라운드 -->
                                <li><a href="/index.php/board/board/<?= $i ?>"><?= $i ?></a></li>
                            <?php } ?>
                        <?php } ?>
